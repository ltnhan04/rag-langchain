...
def remove_non_uf8_characters(text):
    return ''.join(char for char in text if ord(char) < 128)

def load_pdf(pdf_file):
    docs = PyPDFLoader(pdf_file, extract_images=True).load()
    for doc in docs:
        doc.page_content = remove_non_uf8_characters(doc.page_content)
    return docs
...
class TextSplitter: 
    def __init__(self, separators: List[str] = ['\n\n', '\n', '', ' '], chunk_size = 300, chunk_overlap: int = 30) -> None:
...
class Loader:
    def __init__(self, file_type:str = Literal["pdf"], split_kwargs: dict = {
        "chunk_size":300,
        "chunk_overlap":0
    }) -> None:
    ...
    def load_dir(self, dir_path: str, workers:int = 1):
        if self.file_type == "pdf":
            ...
            raise ValueError("file_name must be pdf")
